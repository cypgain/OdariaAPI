...
public class ProxyQuitListener implements Listener {
    ...
    public void onPlayerQuitProxy(PlayerDisconnectEvent event) throws SQLException {
        BungeeCord.getInstance().getScheduler().runAsync(OdariaAPIBungee.INSTANCE, () ->
        {
            ...
            /* Save Account To Database and Remove Account From Redis */
            ...
            /* Save GOO Account To Database And Remove GOO Account From Redis */
            final GOOAccountProvider gooAccountProvider = new GOOAccountProvider(player);
            try {
                gooAccountProvider.saveGOOAccountToDatabase();
            } catch (SQLException e) {
                e.printStackTrace();
            }
            gooAccountProvider.removeGOOAccountFromRedis();
        });
    }
